...
object SeedData {
    ...
    fun getSeedSchemes(): List<SchemeEntity> = schemes
    ...
    val students = listOf(
        ...
        // Persona 4: Dr. S. K. Mahapatra (Reviewing Officer)
        StudentEntity(
            id = "REV_OFFICER_01",
            ...
            qualificationDetails = "Tribal Welfare Reviewer Officer"
        )
    )
    ...
    val officerPersona = StudentEntity(
        id = "REV_OFFICER_01",
        name = "Dr. S. K. Mahapatra (Reviewing Officer)",
        dob = "01-01-1980",
        mobile = "+91 99999 11111",
        ...
        hasConsentGiven = true,
        role = UserRole.REVIEWER.name,
        qualificationDetails = "State Reviewing Officer - Tribal Welfare"
    )

    fun getPresetStudents(): List<StudentEntity> {
        return students + listOf(officerPersona)
    }

    suspend fun populateInitialDatabase(database: EkikritDatabase) = withContext(Dispatchers.IO) {
        database.studentDao().insertAll(getPresetStudents())
